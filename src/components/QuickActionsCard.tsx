...
const QuickActionsCard: React.FC = () => {
  ...
  const isHabitCompleted = (habitId: string) => {
    return todayCompletions.some(completion => completion.habitId === habitId)
  }
    ...
    <div className="bg-card rounded-lg p-3 shadow-sm border border-border">
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="text-right">
          <div className="text-xs text-foreground">{todayCompletions.length}/{habits.length}</div>
      ...
      <div className="space-y-2">
        {habits.length > 0 ? (
          habits.slice(0, 3).map(habit => (  // Show only first 3 habits to save space
