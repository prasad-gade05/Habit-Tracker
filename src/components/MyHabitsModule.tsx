...
const MyHabitsModule: React.FC = () => {
  const { habits, addHabit, isHabitCompletedOnDate, toggleCompletion, updateHabit, deleteHabit } = useHabitStore()
  ...
  return (
    <div className="bg-card rounded-xl p-5 shadow-sm border border-border transition-all duration-300 hover:shadow-md">
      <div className="flex justify-between items-center mb-5">
        ...
        <Button onClick={() => setIsModalOpen(true)} className="bg-accent text-accent-foreground h-8 px-3 text-xs">
      ...
      {habits.length > 0 ? (
        <div className="space-y-3">
          {habits.map((habit: Habit) => {
            ...
            return (
              <div key={habit.id} className="bg-secondary/30 rounded-lg p-4 group transition-all duration-200 hover:bg-secondary/40">
