...
const ContributionChart: React.FC = () => {
  ...
  const todayIndex = useMemo(() => {
    ...
    return last365Days.findIndex(date => 
  ...
  const monthLabels = useMemo(() => {
    ...
    for (let week = 0; week < weeks.length; week++) {
      ...
      if (week === 0 || month !== parseISO(weeks[week - 1]?.find(day => day !== null)?.date || '').getMonth()) {
        labels.push({
          weekIndex: week,
          label: months[month]
        })
          ...
          <div className="flex ml-7 mb-1">
            {monthLabels.map((month, index) => (
              <div 
                ...
                style={{ marginLeft: month.weekIndex > 0 ? `${(month.weekIndex - (monthLabels[index - 1]?.weekIndex || 0) - 1) * 16}px` : 0 }}
              >
                {month.label}
              </div>
            ))}
