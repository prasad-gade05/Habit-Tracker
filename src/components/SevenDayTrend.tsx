...
const SevenDayTrend: React.FC = () => {
  ...
  const averageRate =
    completionData.length > 0
      ? Math.round(
          completionData.reduce((sum, day) => sum + day.percentage, 0) /
  ...
  const todayPercentage =
    completionData.length > 0 ? completionData[6].percentage : 0;
  const yesterdayPercentage =
    completionData.length > 1 ? completionData[5].percentage : 0;
  const trendPercentage =
    yesterdayPercentage > 0
      ? Math.round(
          ((todayPercentage - yesterdayPercentage) / yesterdayPercentage) * 100
        )
      : 0;
