...
import { calculateHabitStreak, isHabitCompletedToday } from "../utils/streakUtils";
...
const HabitStreakVisualization: React.FC = () => {
  ...
  const habitsWithStreaks = useMemo(() => {
    ...
    return habits.map(habit => {
      ...
      const isCompletedToday = isHabitCompletedToday(habit.id, completions, today);
  ...
  const averageStreak = activeHabits.length > 0 
    ? Math.round(activeHabits.reduce((sum, habit) => sum + habit.currentStreak, 0) / activeHabits.length)
    : 0;
    ...
    <div className="bg-card rounded-lg p-6 shadow-sm border border-border">
      ...
      {habits.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p>No habits to track yet. Add some habits to start building streaks!</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-green-500/20 rounded-lg p-3">
              ...
              <div className="text-2xl font-bold text-foreground">{totalActiveStreaks}</div>
            </div>
            <div className="bg-orange-500/20 rounded-lg p-3">
              <div className="text-sm text-orange-500">Avg. Streak</div>
              <div className="text-2xl font-bold text-foreground">{averageStreak} days</div>
            </div>
          </div>

          {topStreakHabits.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-foreground mb-3">Top Streaks</h3>
