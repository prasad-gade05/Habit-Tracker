...
const CompletionRatesVisual: React.FC = () => {
  ...
  const completionRates = useMemo(() => {
    ...
    const todayRate = getCompletionPercentageForDate(
      habits,
      completions,
      todayString
    );
      ...
      last7Days.length > 0
        ? Math.round(
            last7Days.reduce((sum, date) => {
              return (
                sum + getCompletionPercentageForDate(habits, completions, date)
              );
            }, 0) / last7Days.length
          )
        : 0;
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="w-full bg-secondary rounded-full h-2">
            <div
              className={`h-2 rounded-full ${getBgColorClass(
                completionRates.today
              )}`}
